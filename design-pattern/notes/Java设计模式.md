...
* 创建新的对象不教复杂时，可以利用原型模式简化对象创建过程，提高效率。
...
* 优点是比较好理解，简单易操作。
* 设计的程序结构过于简单，没有设计缓冲层对象，程序的扩展和维护性不好。吧产品和创建的过程封装在一起，增加了耦合性。
* 解决方案：将产品和产品见证的过程解耦。

基本介绍：

* 建造者模式又称生成器模式，是一种对象构建模式，它可以将复杂毒系的建造过程抽象出来，使这个抽象过程的不同实现方法可以构造出不同表现的对象。
...
* 使用`java.lang.reflect.Proxy`类的静态方法`static Object newProxyInstance(ClassLoader loader, CLass<?>[] interfaces, InvocationHandler h)`实现代理。
