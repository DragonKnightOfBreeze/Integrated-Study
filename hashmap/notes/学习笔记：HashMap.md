...
通过上述描述，当位于一个链表中的元素较多，即哈希值相等但内容不相等的元素较多时，通过key值依次查找的效率较低。在JDK8中，哈希表存储采用数组+链表+红黑树实现，当链表长多>=阈值8且当前数组的长度>=64时，将链表转换为红黑树。这样大大减少了查找时间。JDK8在哈希表中引入红黑树的原因只是为了提高查找效率。
...
### HashMap底层采用何种算法计算hash值，还有哪些方法可以计算出hash值？
