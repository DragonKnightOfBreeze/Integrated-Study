...
* HashMap当链表长度大于等于8并且数组长度大于等于64时，链表结构将会转换成红黑树。除了添加元素之外，其他操作的效率都会变高。
* ConcurrentHashMap由锁分段转变为CAS无锁算法。效率提高了。
...
在jdk8之后，堆栈中就没有PermGen（永久区）了，取而代之的是MetaSpace（元空间），使用物理内存（RAM）。因此垃圾回收频率变低了。
...
* `Period` 时期。
    * 计算两个日期之间的间隔。
* `Instant` 瞬间。
    * 以Unix元年，即1970.1.1 00:00:00开始，到某个时间之间的毫秒值。
    * 默认获取UTC时区的时间。
