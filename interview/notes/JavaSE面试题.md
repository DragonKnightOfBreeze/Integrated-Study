...
* 实例初始化就是执行`<init>()`方法（JVM生成的不可见方法）
  ...
  * 实例变量显示赋值代码和非静态代码块代码从上到下顺序执行，而对应狗在其的代码最后执行
...
[用迭代实现](../src/main/java/com/windea/study/interview/Question1.java#L25-L41)
...
* 生命周期
  ...
  * 实例比纳凉：随着对象的创建而初始化，随着对象的被回收而消亡，每个对象的实例变量都是独立的
...
当事务方法被另一个事务方法调用时，必须指定事务应该如何传播。例如方法可能继续在现有事务中运行，也可能开启一个新事务，并在自己的事务中允许。
...
* 运行级别2：多用户状态（没有NFS即网络），不支持网络
* 运行级别3：完全的多用户状态（有NFS即网络），登录后进入控制台命令行模式
* 运行级别4：系统未使用，保留
